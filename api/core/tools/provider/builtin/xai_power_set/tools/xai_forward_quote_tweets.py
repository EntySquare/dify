...
XAI_FORWARD_QUOTE_PATH = "adminApi/forwardAndQuoteTwitter"


class XAIForwardAndQuoteTweets(BuiltinTool):
    ...
    def _invoke(self,
        ...
        url = URL(api_url) / "api" / XAI_FORWARD_QUOTE_PATH if 'api' not in api_url else URL(api_url) / XAI_FORWARD_QUOTE_PATH
        ...
        try:
            ...
            if response.status_code == 500:
                raise Exception(f'Failed to distribute task: forward and quote tweets: {tweetsUrl} with user: {accountUsername} and content: {content}')
        except:
            raise Exception(f'Failed to distribute task: forward and quote tweets: {tweetsUrl} with user: {accountUsername} and content: {content}')
        
        if response.status_code == 200 and isinstance(response.text, str):
            return self.create_text_message(f'Task: User {accountUsername} forward and quote tweets {tweetsUrl} with content {content} distribute successfully!')
        else:
            raise Exception(f'Failed to distribute task: forward and quote tweets: {tweetsUrl} with user: {accountUsername} and content: {content}')
