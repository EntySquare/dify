...
XAI_UPDATE_SEG_PATH = "knowledge/updateSeg"


class XAIUpdateSeg(BuiltinTool):
    ...
    def _invoke(self,
        ...
        segment_string = tool_parameters.get("segment", "")
        ...
        if segment_id is None or segment_id == "":
            raise Exception("分段 ID不能为空！")
        
        if segment_string is None or segment_string == "":
            raise Exception("分段内容不能为空!")
        
        try:
            segment = json.loads(segment_string)

            if isinstance(segment, dict) is not True or 'segment' not in segment:
        ...
        url = URL(api_url) / "api" / XAI_UPDATE_SEG_PATH if 'api' not in api_url else URL(api_url) / XAI_UPDATE_SEG_PATH
        ...
        try:
            ...
            raise Exception(f'Failed to update seg {segment_id} with content {segment_string} into {document_id} of {dataset_id}')
