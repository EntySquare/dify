...
XAI_GENERATE_REPLY_TEMPLATE_PATH = "adminApi/chat/template/replyDmsArr"


class XAIGenerateReplyResponseTemplate(BuiltinTool):
    ...
    def _invoke(self,
        ...
        url = URL(api_url) / "api" / XAI_GENERATE_REPLY_TEMPLATE_PATH if 'api' not in api_url else URL(api_url) / XAI_GENERATE_REPLY_TEMPLATE_PATH
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                raise Exception(f'Failed to generate response template(generate reply) with {json_string}')
