...
import FloatPopoverContainer from '../../../../components/base/float-popover-container'
...
const LikedItem = ({
  ...
  return (
    <Link className={classNames(s.itemWrapper, 'px-2 dark:hover:!bg-zinc-600', isMobile && 'justify-center')} href={`/app/${detail?.id}/overview`}>
...
const TargetIcon = ({ className }: SVGProps<SVGElement>) => {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className={className ?? ''}>
    <g clipPath="url(#clip0_4610_6951)">
      <path d="M10.6666 5.33325V3.33325L12.6666 1.33325L13.3332 2.66659L14.6666 3.33325L12.6666 5.33325H10.6666ZM10.6666 5.33325L7.9999 7.99988M14.6666 7.99992C14.6666 11.6818 11.6818 14.6666 7.99992 14.6666C4.31802 14.6666 1.33325 11.6818 1.33325 7.99992C1.33325 4.31802 4.31802 1.33325 7.99992 1.33325M11.3333 7.99992C11.3333 9.84087 9.84087 11.3333 7.99992 11.3333C6.15897 11.3333 4.66659 9.84087 4.66659 7.99992C4.66659 6.15897 6.15897 4.66659 7.99992 4.66659" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
    </g>
    <defs>
...
const ExtraInfo = ({ isMobile, relatedApps }: IExtraInfoProps) => {
  ...
  return <div className='w-full flex flex-col items-center'>
    <Divider className='mt-5 dark:!bg-zinc-600' />
    {(relatedApps?.data && relatedApps?.data?.length > 0) && (
      <>
        {!isMobile && <div className='w-full px-2 pb-1 pt-4 uppercase text-xs text-tgai-text-3 font-medium'>{relatedApps?.total || '--'} {t('common.datasetMenus.relatedApp')}</div>}
        {isMobile && <div className={classNames(s.subTitle, 'flex items-center justify-center !px-0 gap-1')}>
          {relatedApps?.total || '--'}
          <PaperClipIcon className='h-4 w-4 text-tgai-text-2' />
        </div>}
    ...
    {!relatedApps?.data?.length && (
      <FloatPopoverContainer
        ...
        isMobile={isMobile}
        triggerElement={
          <div className={classNames('h-7 w-7 inline-flex justify-center items-center rounded-lg bg-transparent', isShowTips && '!bg-gray-50 dark:!bg-zinc-600')}>
            <QuestionMarkCircleIcon className='h-4 w-4 flex-shrink-0 text-tgai-text-3' />
          </div>
        }
      >
        <div className={classNames('mt-5 p-3', isMobile && 'border-[0.5px] border-gray-200 dark:border-stone-600 shadow-lg dark:shadow-stone-800 rounded-lg bg-white dark:bg-tgai-panel-background-3 w-[160px]')}>
...
const DatasetDetailLayout: FC<IAppDetailLayoutProps> = (props) => {
  ...
  useEffect(() => {
    if (datasetRes)
      document.title = `${datasetRes.name || 'Dataset'} - TGAI`
  }, [datasetRes])
  ...
  return (
    <div className='grow flex overflow-hidden'>
      ...
      <DatasetDetailContext.Provider value={{
        ...
        <div className="bg-tgai-panel-background grow overflow-hidden">{children}</div>
