...
import { XAIPost } from "@/service/x-http";
...
export default function MailAndPasswordAuth({
  ...
  const handleEmailPasswordLogin = async () => {
    ...
    try {
      ...
      const TGAILoginRes = await XAIPost<{ token: string; userInfo: any }>(
        "/manage/login",
        {
          username: email,
          password,
      ...
      if (res.result === "success" && TGAILoginRes.data.token) {
        if (isInvite) {
          ...
          localStorage.setItem("xai_token", TGAILoginRes.data.token);
          localStorage.setItem(
            "xai_user_info",
            JSON.stringify(TGAILoginRes.data.userInfo)
          );
          router.replace("/group/control");
        }
      }
