...
const IndexBar: FC<IndexBarProps> = ({ letters, itemRefs }) => {
  ...
  return (
    <div className="index-bar fixed right-4 top-36 flex flex-col items-center text-xs font-medium text-tgai-text-2">
      {letters.map(letter => (
        <div className="hover:text-tgai-text-1 cursor-pointer" key={letter} onClick={() => handleIndexClick(letter)}>
          {letter}
        </div>
      ))}
    </div>
