...
const NodePanel: FC<Props> = ({
  ...
  return (
    <div className={cn('px-4 py-1', className, hideInfo && '!p-0')}>
      <div className={cn('group transition-all bg-tgai-panel-background border border-gray-100 dark:border-stone-700 rounded-2xl shadow-xs dark:shadow-stone-800 hover:shadow-md', hideInfo && '!rounded-lg')}>
        <div
          className={cn(
        ...
        >
          {!hideProcessDetail && (
            <RiArrowRightSLine
              className={cn(
                'shrink-0 w-3 h-3 mr-1 text-tgai-text-3 transition-all group-hover:text-tgai-text-2',
          ...
          <div className={cn(
            'grow text-tgai-text-2 text-[13px] leading-[16px] font-semibold truncate',
            hideInfo && '!text-xs',
          )} title={nodeInfo.title}>{nodeInfo.title}</div>
          {nodeInfo.status !== 'running' && !hideInfo && (
            <div className='shrink-0 text-tgai-text-3 text-xs leading-[18px]'>{`${getTime(nodeInfo.elapsed_time || 0)} · ${getTokenCount(nodeInfo.execution_metadata?.total_tokens || 0)} tokens`}</div>
          )}
          ...
          {nodeInfo.status === 'running' && (
            <div className='shrink-0 flex items-center text-tgai-primary text-[13px] leading-[16px] font-medium'>
          ...
          <div className='pb-2'>
            ...
            {isIterationNode && !notShowIterationNav && (
              <div className='mt-2 mb-1 !px-2'>
                <div
                  className='flex items-center h-[34px] justify-between px-3 bg-gray-100 dark:bg-tgai-input-background border-[0.5px] border-gray-200 dark:border-stone-600 rounded-lg cursor-pointer'
                  onClick={handleOnShowIterationDetail}>
                  <div className='leading-[18px] text-[13px] font-medium text-tgai-text-2'>{t('workflow.nodes.iteration.iteration', { count: nodeInfo.metadata?.iterator_length || nodeInfo.details?.length })}</div>
                  {justShowIterationNavArrow
                    ? (
            ...
            <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
              {nodeInfo.status === 'stopped' && (
                <div className='px-3 py-[10px] bg-[#fffaeb] dark:bg-orange-800 rounded-lg border-[0.5px] border-[rbga(0,0,0,0.05)] dark:border-stone-600/95 text-xs leading-[18px] text-[#dc6803] shadow-xs dark:shadow-stone-800'>{t('workflow.tracing.stopBy', { user: nodeInfo.created_by ? nodeInfo.created_by.name : 'N/A' })}</div>
              )}
              {nodeInfo.status === 'failed' && (
                <div className='px-3 py-[10px] bg-[#fef3f2] dark:bg-red-800 rounded-lg border-[0.5px] border-[rbga(0,0,0,0.05)] dark:border-stone-600/95 text-xs leading-[18px] text-[#d92d20] shadow-xs dark:shadow-stone-800'>{nodeInfo.error}</div>
              )}
            </div>
            {nodeInfo.inputs && (
              <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
