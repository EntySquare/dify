...
const StepNum: FC<{ children: React.ReactNode }> = ({ children }) =>
  <div className='h-7 w-7 flex justify-center items-center flex-shrink-0 mr-3 text-primary-600 dark:text-tgai-primary bg-primary-50 dark:bg-neutral-700 rounded-2xl'>
    {children}
  </div>
...
const CustomizeModal: FC<IShareLinkProps> = ({
  ...
  return <Modal
    ...
    isShow={isShow}
    onClose={onClose}
    className='!max-w-2xl w-[640px] dark:bg-tgai-panel-background dark:border dark:border-stone-600'
    closable={true}
  >
    <div className='w-full mt-4 px-6 py-5 border-gray-200 dark:border-stone-600 rounded-lg border-[0.5px]'>
      <Tag bordered={true} hideBg={true} className='text-primary-600 dark:text-tgai-primary border-primary-600 dark:border-tgai-primary uppercase'>{t(`${prefixCustomize}.way`)} 1</Tag>
      <p className='my-2 text-base font-medium text-tgai-text-1'>{t(`${prefixCustomize}.way1.name`)}</p>
      <div className='flex py-4'>
        <StepNum>1</StepNum>
        <div className='flex flex-col'>
          <div className='text-tgai-text-1'>{t(`${prefixCustomize}.way1.step1`)}</div>
          <div className='text-tgai-text-3 text-xs mt-1 mb-2'>{t(`${prefixCustomize}.way1.step1Tip`)}</div>
          ...
          <a href={`javascript:void(0)`} target='_blank' rel='noopener noreferrer'>
            {/* <Button><GithubIcon className='text-tgai-text-1 mr-2' />{t(`${prefixCustomize}.way1.step1Operation`)}</Button> */}
            <Button><GithubIcon className='text-tgai-text-1 mr-2' />{'TGAI-WebClient'}</Button>
          </a>
        </div>
      </div>
      <div className='flex pt-4'>
        <StepNum>2</StepNum>
        <div className='flex flex-col'>
          <div className='text-tgai-text-1'>{t(`${prefixCustomize}.way1.step3`)}</div>
          <div className='text-tgai-text-3 text-xs mt-1 mb-2'>{t(`${prefixCustomize}.way1.step2Tip`)}</div>
          <a href="https://vercel.com/docs/concepts/deployments/git/vercel-for-github" target='_blank' rel='noopener noreferrer'>
            <Button>
              <div className='mr-1.5 border-solid border-t-0 border-r-[7px] border-l-[7px] border-b-[12px] border-r-transparent border-b-tgai-text-1 border-l-transparent border-t-transparent'></div>
      ...
      <div className='flex py-4'>
        ...
        <div className='flex flex-col w-full overflow-hidden'>
          <div className='text-tgai-text-1'>{t(`${prefixCustomize}.way1.step3`)}</div>
          <div className='text-tgai-text-3 text-xs mt-1 mb-2'>{t(`${prefixCustomize}.way1.step3Tip`)}</div>
          <pre className='overflow-x-scroll tgai-custom-hor-scrollbar box-border py-3 px-4 bg-gray-100 dark:bg-tgai-input-background text-tgai-text-1 text-xs font-medium rounded-lg select-text'>
    ...
    <div className='w-full mt-4 px-6 py-5 border-gray-200 dark:border-stone-600 rounded-lg border-[0.5px]'>
      <Tag bordered={true} hideBg={true} className='text-primary-600 dark:text-tgai-primary border-primary-600 dark:border-tgai-primary uppercase'>{t(`${prefixCustomize}.way`)} 2</Tag>
      <p className='mt-2 text-base font-medium text-tgai-text-1'>{t(`${prefixCustomize}.way2.name`)}</p>
      <Button
        className='mt-2'
        // onClick={() =>
        //   window.open(
        ...
        // }
      >
        <span className='text-sm text-tgai-text-1'>{t(`${prefixCustomize}.way2.operation`)}</span>
        <ArrowTopRightOnSquareIcon className='w-4 h-4 ml-1 text-tgai-text-1 shrink-0' />
      </Button>
    </div>
  </Modal>
}
