    ...
    <>
      {
        forms.map((form, index) => (
          <div
            ...
            className='py-2'
          >
            <div className='flex items-center h-9 text-sm font-medium text-tgai-text-1'>
            ...
            }
            {
              form.type === 'paragraph' && (
                <div className='relative px-3 py-2 h-[88px] bg-gray-100 dark:bg-tgai-input-background rounded-lg'>
                  <textarea
                    value={value?.[form.variable] || ''}
                    className='block w-full h-full bg-transparent text-sm text-tgai-text-1 outline-none appearance-none resize-none'
                    placeholder={form.placeholder}
                    onChange={e => handleFormChange(form.variable, e.target.value)}
                  />
                </div>
