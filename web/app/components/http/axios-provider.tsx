...
import { XAIHttp } from '@/service/x-http'
...
export const AxiosProvider = ({ children }: AxiosProviderProps) => {
  ...
  useEffect(() => {
    ...
    const xaiTokenInterceptorsId = XAIHttp.interceptors.response.use(tokenExpirationInterceptors, axiosErrorInterceptors)

    return () => {
      TGAIHttp.interceptors.response.eject(tokenInterceptorsId)
      XAIHttp.interceptors.response.eject(xaiTokenInterceptorsId)
    }
  }, [])
