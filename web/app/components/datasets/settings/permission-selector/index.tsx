    ...
    >
      <div className='relative'>
        ...
        >
          {permission === 'only_me' && (
            <div className={cn(
                ...
                disabled && 'hover:!bg-gray-100 dark:hover:!bg-tgai-input-background !cursor-default',
              )}
            >
        ...
        <PortalToFollowElemContent className='z-[1002]'>
          <div className='relative w-[480px] rounded-lg border-[0.5px] bg-white dark:bg-tgai-section-background shadow-lg dark:shadow-stone-800'>
            <div className='p-1'>
              <div className='pl-3 pr-2 py-1 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-600 cursor-pointer' onClick={() => {
                onChange('only_me')
                setOpen(false)
              }}>
                <div className='flex items-center gap-2'>
                  <Avatar name={userProfile.name} className='shrink-0 mr-2' size={24} />
                  <div className='grow mr-2 text-tgai-text-1 text-sm leading-5'>{t('datasetSettings.form.permissionsOnlyMe')}</div>
                  {permission === 'only_me' && <Check className='w-4 h-4 text-tgai-primary' />}
                </div>
              </div>
              <div className='pl-3 pr-2 py-1 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-600 cursor-pointer' onClick={() => {
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='grow mr-2 text-tgai-text-1 text-sm leading-5'>{t('datasetSettings.form.permissionsAllMember')}</div>
                  {permission === 'all_team_members' && <Check className='w-4 h-4 text-tgai-primary' />}
                </div>
              </div>
              <div className='pl-3 pr-2 py-1 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-600 cursor-pointer' onClick={() => {
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='grow mr-2 text-tgai-text-1 text-sm leading-5'>{t('datasetSettings.form.permissionsInvitedMembers')}</div>
                  {permission === 'partial_members' && <Check className='w-4 h-4 text-tgai-primary' />}
                </div>
              </div>
            </div>
            {permission === 'partial_members' && (
                  ...
                  <div className='pl-3 pr-[10px] py-1 flex gap-2 items-center rounded-lg'>
                    ...
                    <div className='grow'>
                      <div className='text-[13px] text-tgai-text-2 font-medium leading-[18px] truncate'>
                        {userProfile.name}
                        <span className='text-xs text-tgai-text-3 font-normal'>{t('datasetSettings.form.me')}</span>
                      </div>
                      <div className='text-xs text-tgai-text-3 leading-[18px] truncate'>{userProfile.email}</div>
                    </div>
                    <Check className='shrink-0 w-4 h-4 text-tgai-primary opacity-30' />
                  </div>
                )}
                {filteredMemberList.map(member => (
                  <div key={member.id} className='pl-3 pr-[10px] py-1 flex gap-2 items-center rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-600 cursor-pointer' onClick={() => selectMember(member)}>
                    <Avatar name={member.name} className='shrink-0' size={24} />
                    <div className='grow'>
                      <div className='text-[13px] text-tgai-text-2 font-medium leading-[18px] truncate'>{member.name}</div>
                      <div className='text-xs text-tgai-text-3 leading-[18px] truncate'>{member.email}</div>
                    </div>
                    {value.includes(member.id) && <Check className='shrink-0 w-4 h-4 text-tgai-primary' />}
                  </div>
                ))}
              </div>
            )}
