...
const TagRemoveModal = ({ show, tag, onConfirm, onClose }: TagRemoveModalProps) => {
  ...
  return (
    <Modal
      className={cn('p-8 max-w-[480px] w-[480px] !bg-tgai-panel-background border-tgai-panel-border border', s.bg)}
      isShow={show}
      onClose={() => { }}
    >
      ...
      <div className='mt-3 text-xl font-semibold leading-[30px] text-tgai-text-1'>
        {`${t('common.tag.delete')} `}
        <span>{`"${tag.name}"`}</span>
      </div>
      <div className='my-1 text-tgai-text-2 text-sm leading-5'>
