    ...
    <div className={cn('w-56 text-right', outerContainerClassName)}>
      <Menu as="div" className="relative inline-block text-left">
        <div>
          ...
          text-tgai-text-1 text-[13px] font-medium
        ...
        >
          <Menu.Items className="absolute right-0 mt-2 w-[200px] origin-top-right divide-y divide-gray-100 rounded-md bg-white dark:bg-tgai-input-background shadow-lg ring-1 ring-black dark:ring-stone-600 ring-opacity-5 focus:outline-none z-10">
            <div className="px-1 py-1 ">
              {items.map((item) => {
                return <Menu.Item key={item.value}>
                  {({ active }) => (
                    <button
                      className={`${active ? 'bg-gray-100 dark:!bg-zinc-600' : ''
                      } group flex w-full items-center rounded-lg px-3 py-2 text-sm text-tgai-text-2`}
                      onClick={(evt) => {
                        evt.preventDefault()
