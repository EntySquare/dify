  ...
  return (
    <div
      className={cn(
        ...
        className,
        isChosen &&
          "bg-components-option-card-option-selected-bg border-components-panel-border shadow-xs",
      )}
    >
      <div className="flex py-3 pl-3 pr-4" onClick={onChosen}>
        <div
          className={cn(
            iconBgClassName,
            "mr-3 shrink-0 flex w-8 justify-center h-8 items-center rounded-lg",
          )}
        >
          {icon}
        </div>
        ...
        {!noRadio && (
          <div className="shrink-0 flex items-center h-8">
            <div
              className={cn(
                "w-4 h-4 border border-components-radio-border dark:border-stone-600 bg-components-radio-bg shadow-xs rounded-full",
                isChosen &&
                  "border-[5px] border-components-radio-border-checked dark:border-tgai-primary",
              )}
            ></div>
          </div>
        )}
      ...
      {((isChosen && chosenConfig) || noRadio) && (
        <div
          className={cn(
            chosenConfigWrapClassName,
            "p-3 border-t border-gray-200 dark:border-stone-600",
          )}
        >
          {chosenConfig}
        </div>
