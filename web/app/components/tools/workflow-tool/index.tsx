...
import LabelSelector from '../labels/selector'
  ...
  return (
    <>
      <Drawer
        ...
        headerClassName='!border-b-black/5 dark:!border-stone-600/95'
        body={
          <div className='flex flex-col h-full'>
            <div className='grow h-0 overflow-y-auto tgai-custom-scrollbar px-6 py-3 space-y-4'>
              {/* name & icon */}
              <div>
                <div className='py-2 leading-5 text-sm font-medium text-tgai-text-1'>{t('tools.createTool.name')} <span className='ml-1 text-red-500'>*</span></div>
                <div className='flex items-center justify-between gap-3'>
                  <AppIcon size='large' onClick={() => { setShowEmojiPicker(true) }} className='cursor-pointer' icon={emoji.content} background={emoji.background} />
                  <input
                    type='text'
                    className='grow h-10 px-3 text-sm font-normal bg-gray-100 dark:bg-tgai-input-background text-tgai-text-1 rounded-lg border border-transparent outline-none appearance-none caret-tgai-primary placeholder:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-700 hover:border hover:border-gray-300 dark:hover:border-zinc-600 focus:bg-gray-50 dark:focus:bg-zinc-700 focus:border focus:border-gray-300 dark:focus:border-zinc-600 focus:shadow-xs dark:focus:shadow-zinc-800'
              ...
              {/* name for tool call */}
              <div>
                <div className='flex items-center py-2 leading-5 text-sm font-medium text-tgai-text-1'>
                  ...
                  >
                    <RiQuestionLine className='ml-2 w-[14px] h-[14px] text-tgai-text-3' />
                  </Tooltip>
                </div>
                <input
                  type='text'
                  className='w-full h-10 px-3 text-sm font-normal bg-gray-100 dark:bg-tgai-input-background text-tgai-text-1 rounded-lg border border-transparent outline-none appearance-none caret-tgai-primary placeholder:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-700 hover:border hover:border-gray-300 dark:hover:border-zinc-600 focus:bg-gray-50 dark:focus:bg-zinc-700 focus:border focus:border-gray-300 dark:focus:border-zinc-600 focus:shadow-xs dark:focus:shadow-zinc-800'
              ...
              {/* description */}
              <div>
                <div className='py-2 leading-5 text-sm font-medium text-tgai-text-1'>{t('tools.createTool.description')}</div>
                <textarea
                  className='w-full px-3 py-2 text-sm font-normal text-tgai-text-1 bg-gray-100 dark:bg-tgai-input-background rounded-lg border border-transparent outline-none appearance-none caret-tgai-primary placeholder:text-gray-400 hover:bg-gray-50 dark:hover:bg-zinc-700 hover:border hover:border-gray-300 dark:hover-border-zinc-600 focus:bg-gray-50 dark:focus:bg-zinc-700 focus:border focus:border-gray-300 dark:focus:border-zinc-600 focus:shadow-xs h-[80px] dark:focus:shadow-zinc-800 resize-none'
              ...
              {/* Tool Input  */}
              <div>
                <div className='py-2 leading-5 text-sm font-medium text-tgai-text-1'>{t('tools.createTool.toolInput.title')}</div>
                <div className='rounded-lg border border-gray-200 dark:border-stone-600 w-full overflow-x-auto'>
                  <table className='w-full leading-[18px] text-xs text-tgai-text-2 font-normal'>
                    ...
                    <tbody>
                      {parameters.map((item, index) => (
                        <tr key={index} className='border-b last:border-0 border-gray-200 dark:border-stone-600'>
                          <td className="p-2 pl-3 max-w-[156px]">
                            <div className='text-[13px] leading-[18px]'>
                              <div title={item.name} className='flex'>
                                <span className='font-medium text-tgai-text-1 truncate'>{item.name}</span>
                                <span className='shrink-0 pl-1 text-[#ec4a0a] text-xs leading-[18px]'>{item.required ? t('tools.createTool.toolInput.required') : ''}</span>
                              </div>
                              <div className='text-tgai-text-3'>{item.type}</div>
                            </div>
                          </td>
                          <td>
                            {item.name === '__image' && (
                              <div className={cn(
                                'flex items-center gap-1 min-h-[56px] px-3 py-2 h-9 bg-white dark:bg-tgai-panel-background cursor-default',
                              )}>
                          ...
                          <td className="p-2 pl-3 text-tgai-text-3 w-[236px]">
                            <input
                              type='text'
                              className='grow text-tgai-text-2 text-[13px] leading-[18px] font-normal bg-white dark:bg-tgai-panel-background outline-none appearance-none caret-tgai-primary placeholder:text-gray-400'
              ...
              {/* Tags */}
              <div>
                <div className='py-2 leading-5 text-sm font-medium text-tgai-text-1'>{t('tools.createTool.toolInput.label')}</div>
                <LabelSelector value={labels} onChange={handleLabelSelect} />
              </div>
              {/* Privacy Policy */}
              <div>
                <div className='py-2 leading-5 text-sm font-medium text-tgai-text-1'>{t('tools.createTool.privacyPolicy')}</div>
                <input
                  value={privacyPolicy}
            ...
            <div className={cn((!isAdd && onRemove) ? 'justify-between' : 'justify-end', 'mt-2 shrink-0 flex py-4 px-6 rounded-b-[10px] bg-gray-50 dark:bg-tgai-panel-background-3 border-t border-black/5 dark:border-stone-600/95')} >
              {!isAdd && onRemove && (
