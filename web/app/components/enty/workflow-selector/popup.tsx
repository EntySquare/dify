...
const Popup = React.memo<Props>(({ workflowList, selectedWorkflow, onSelect }) => {
  const [searchText, setSearchText] = useState('')

  const filteredWorkflowList = workflowList.filter((workflow) => {
    return workflow.workflow_name.toLowerCase().includes(searchText.toLowerCase())
  })
      ...
      <div className='p-1'>
        {
          filteredWorkflowList.map(workflow => (
            <PopupItem
              ...
              workflow={workflow}
              selected={selectedWorkflow === workflow.workflow_id}
              onSelect={onSelect}
            />
          ))
        }
        {
          !filteredWorkflowList.length && (
...
Popup.displayName = 'WorkflowSelectorPopup'
