...
import { getAllPeriodicalTasks, getXAIAllWorkflows } from '@/service/xai'
...
const PeriodicalTasksContainer: FC = () => {
  ...
  const { data: arrangementList } = useSWR(['/workflow/all'], getXAIAllWorkflows)
  ...
  const currentTaskData = useMemo(() => {
    if (!currentTask || !data || !data.data.cron_list)
      return undefined
