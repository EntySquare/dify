...
{/* Arrangement -> Workflow; arrangement -> workflow */}
# Arrangement 应用 API

Arrangement 应用无会话支持，适合用于翻译/文章写作/总结 AI 等等。
...
<Heading
  url='/workflows/run'
  method='POST'
  title='执行 arrangement'
  name='#Execute-Arrangement'
/>
<Row>
  <Col>
    执行 arrangement，没有已发布的 arrangement，不可执行。
    ...
    - `workflow_run_id` (string) arrangement 执行 ID
    - `task_id` (string) 任务 ID，用于请求跟踪和下方的停止响应接口
    - `data` (object) 详细内容
      - `id` (string) arrangement 执行 ID
      - `workflow_id` (string) 关联 Arrangement ID
    ...
    - `event: workflow_started` arrangement 开始执行
      - `task_id` (string) 任务 ID，用于请求跟踪和下方的停止响应接口
      - `workflow_run_id` (string) arrangement 执行 ID
    ...
    - `event: node_finished` node 执行结束，成功失败同一事件中不同状态
      ...
      - `data` (object) 详细内容
        ...
        - `created_at` (timestamp) 开始时间
    - `event: workflow_finished` arrangement 执行结束，成功失败同一事件中不同状态
      - `task_id` (string) 任务 ID，用于请求跟踪和下方的停止响应接口
      - `workflow_run_id` (string) arrangement 执行 ID
    ...
    - 400，`workflow_request_error`，arrangement 执行失败
    - 500，服务内部异常
...
<Heading
  ...
  method='GET'
  title='获取arrangement执行情况'
  name='#get-arrangement-run-detail'
/>
<Row>
  <Col>
    根据 arrangement 执行 ID 获取 arrangement 任务当前执行结果
    ### Path
    - `workflow_id` (string) arrangement 执行 ID，可在流式返回 Chunk 中获取
    ### Response
    - `id` (string) arrangement 执行 ID
    - `workflow_id` (string) 关联的 Arrangement ID
    - `status` (string) 执行状态 `running` / `succeeded` / `failed` / `stopped`
    - `inputs` (json) 任务输入内容
    - `outputs` (json) 任务输出内容
    - `error` (string) 错误原因
