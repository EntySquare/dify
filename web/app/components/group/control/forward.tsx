...
export type ForwardModalType = string;

type ForwardModalProps = {
  //   tweetsUrl: string | undefined;
};

export type ForwardModalRefType = {
  show: () => Promise<ForwardModalType | false>;
};

const ForwardModal = React.forwardRef<ForwardModalRefType, ForwardModalProps>(
  ({}, ref) => {
    ...
    const promiseRef = useRef<{
      resolve: (value: ForwardModalType | false) => void;
    }>();
    ...
    const handleConfirm = async () => {
      try {
        ...
        await forwardTwitter(selectedUsernames, searchUrl);
        form.resetFields();
        setVisible(false);
        Message.success({
