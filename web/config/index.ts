...
if (
  process.env.NEXT_PUBLIC_TGAI_API_PREFIX
  && process.env.NEXT_PUBLIC_TGAI_WS_PREFIX
) {
  ...
  // Not build can not get env from process.env.NEXT_PUBLIC_ in browser https://nextjs.org/docs/basic-features/environment-variables#exposing-environment-variables-to-the-browser
  TGAIApiPrefix = globalThis.document.body.getAttribute(
    'tgai-http-url',
  ) as string
...
if (
  process.env.NEXT_PUBLIC_XAI_API_PREFIX
) {
  XAIApiPrefix = process.env.NEXT_PUBLIC_XAI_API_PREFIX
}
else if (
  globalThis.document?.body?.getAttribute('xai-http-url')
) {
  // Not build can not get env from process.env.NEXT_PUBLIC_ in browser https://nextjs.org/docs/basic-features/environment-variables#exposing-environment-variables-to-the-browser
  XAIApiPrefix = globalThis.document.body.getAttribute(
    'xai-http-url',
  ) as string
  XAIApiPrefix = globalThis.document.body.getAttribute('xai-http-url') as string
}
else {
  XAIApiPrefix = 'http://127.0.0.1:3021/api'
}
...
export const XAI_API_PREFIX: string = XAIApiPrefix
...
if (
  ...
  textGenerationTimeoutMs = parseInt(
    process.env.NEXT_PUBLIC_TEXT_GENERATION_TIMEOUT_MS,
  )
}
else if (
